...
async function send({ to, subject, html }) {
  if (!process.env.BREVO_API_KEY || !process.env.BREVO_SENDER_EMAIL) return { skipped: true };
  const response = await fetch('https://api.brevo.com/v3/smtp/email', {
    method: 'POST',
    headers: { accept: 'application/json', 'api-key': process.env.BREVO_API_KEY, 'content-type': 'application/json' },
    body: JSON.stringify({ sender: { name: process.env.BREVO_SENDER_NAME || 'Mustafa & Arwa', email: process.env.BREVO_SENDER_EMAIL }, to: [{ email: to }], subject, htmlContent: html })
  });
