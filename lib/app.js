...
app.get('/api/health', (_req,res)=>res.json({ok:true,emailProvider:process.env.BREVO_API_KEY&&process.env.BREVO_SENDER_EMAIL?'brevo-api':process.env.SMTP_HOST&&process.env.SMTP_USER&&process.env.SMTP_PASS&&process.env.SMTP_FROM_EMAIL?'smtp':'unconfigured'}));
...
app.use((error,req,res,_next)=>{console.error('[api] request failed',{method:req.method,path:req.path,message:error.message,stack:error.stack});res.status(500).json({message:'The server could not complete that request. Please try again in a moment.'})});
