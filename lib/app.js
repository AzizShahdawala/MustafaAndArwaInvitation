...
app.get('/api/admin/rsvps',auth,async(_req,res,next)=>{try{await connectDb();const rsvps=await RSVP.find().sort({createdAt:-1}).lean();res.json({rsvps,summary:{responses:rsvps.length,attending:rsvps.filter(r=>r.attending==='yes').length,guests:rsvps.reduce((n,r)=>n+r.guestCount,0),declined:rsvps.filter(r=>r.attending==='no').length}})}catch(e){next(e)}});
app.post('/api/admin/reminders/:id',auth,async(req,res,next)=>{try{await connectDb();const rsvp=await RSVP.findById(req.params.id);if(!rsvp)return res.status(404).json({message:'Guest not found.'});await sendReminder(rsvp);rsvp.reminderSentAt=new Date();await rsvp.save();res.json({message:'Reminder sent.'})}catch(e){next(e)}});
